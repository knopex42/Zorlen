...
Zorlen_Druid_FileBuildNumber = 688
...
function isTravelForm(unit)
	if not unit or unit == "player" then
		if LOCALIZATION_ZORLEN.TravelForm == Zorlen_CurrentForm then
			return true
		end
	else
		return Zorlen_checkBuffByName(LOCALIZATION_ZORLEN.TravelForm, unit)
	end
	return false
end
...
function isMoonkinForm(unit)
	if not unit or unit == "player" then
		if LOCALIZATION_ZORLEN.MoonkinForm == Zorlen_CurrentForm then
			return true
		end
	else
		return Zorlen_checkBuffByName(LOCALIZATION_ZORLEN.MoonkinForm, unit)
	end
	return false
end
...
function isBearForm(unit)
	if not unit or unit == "player" then
		if LOCALIZATION_ZORLEN.BearForm == Zorlen_CurrentForm then
			return true
		end
	else
		return Zorlen_checkBuffByName(LOCALIZATION_ZORLEN.BearForm, unit)
	end
	return false
end
...
function isDireBearForm(unit)
	if not unit or unit == "player" then
		if LOCALIZATION_ZORLEN.DireBearForm == Zorlen_CurrentForm then
			return true
		end
	else
		return Zorlen_checkBuffByName(LOCALIZATION_ZORLEN.DireBearForm, unit)
	end
	return false
end
...
function isCatForm(unit)
	if not unit or unit == "player" then
		if LOCALIZATION_ZORLEN.CatForm == Zorlen_CurrentForm then
			return true
		end
	else
		return Zorlen_checkBuffByName(LOCALIZATION_ZORLEN.CatForm, unit)
	end
	return false
end
...
function isAquaticForm(unit)
	if not unit or unit == "player" then
		if LOCALIZATION_ZORLEN.AquaticForm == Zorlen_CurrentForm then
			return true
		end
	else
		return Zorlen_checkBuffByName(LOCALIZATION_ZORLEN.AquaticForm, unit)
	end
	return false
end
...
function isFaerieFire(unit, dispelable)
	local SpellName = LOCALIZATION_ZORLEN.FaerieFire
...
function Zorlen_Druid_SpellTimerSet()
	...
	if SpellName == LOCALIZATION_ZORLEN.Moonfire then
	...
	elseif SpellName == LOCALIZATION_ZORLEN.InsectSwarm then
		Number = 12
		
	elseif SpellName == LOCALIZATION_ZORLEN.Regrowth then
		if Zorlen_CastingSpellTargetName then
			DebuffName = SpellName
			DebuffTargetName = TargetName
		end
		Zorlen_SetTimer(1, SpellName, TargetName, "InternalZorlenSpellCastDelay", 2)
...
function castTravelForm()
	local SpellName = LOCALIZATION_ZORLEN.TravelForm
...
function castMoonkinForm()
	local SpellName = LOCALIZATION_ZORLEN.MoonkinForm
	local EnemyTargetNotNeeded = 1
	local SpellCheckNotNeeded = 1
	if Zorlen_CheckDruidForm(SpellName) then
		return false
...
function castBearForm()
	local SpellName = LOCALIZATION_ZORLEN.BearForm
	local EnemyTargetNotNeeded = 1
	local SpellCheckNotNeeded = 1
	local spellknown = Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN.DireBearForm)
	if spellknown then
		SpellName = LOCALIZATION_ZORLEN.DireBearForm
	end
	if Zorlen_CheckDruidForm(SpellName) then
		return false
...
function castCatForm()
	local SpellName = LOCALIZATION_ZORLEN.CatForm
	local EnemyTargetNotNeeded = 1
	local SpellCheckNotNeeded = 1
	if Zorlen_CheckDruidForm(SpellName) then
		return false
...
function castAquaticForm()
	local SpellName = LOCALIZATION_ZORLEN.AquaticForm
	local EnemyTargetNotNeeded = 1
	local SpellCheckNotNeeded = 1
	if Zorlen_CheckDruidForm(SpellName) then
		return false
...
function castGroupHealingTouch(pet, Mode, RankAdj)
	local SpellName = LOCALIZATION_ZORLEN.HealingTouch
	local LowestMana = 25
	local t = Zorlen_GetTalentRank(LOCALIZATION_ZORLEN.TranquilSpirit)
	if t and t > 0 then
...
function castHealingTouch(Mode, RankAdj, unit)
	...
	if Zorlen_IsSpellKnown(SpellName) and not isCasterForm() then
		...
		if Zorlen_HasTalent(LOCALIZATION_ZORLEN.Moonglow) then
...
function castGroupRegrowth(pet, Mode, RankAdj)
	local SpellName = LOCALIZATION_ZORLEN.Regrowth
...
function castGroupRejuvenation(pet, Mode, RankAdj)
	local SpellName = LOCALIZATION_ZORLEN.Rejuvenation
...
function castRejuvenation(Mode, RankAdj, unit)
	...
	if Zorlen_IsSpellKnown(SpellName) and not isCasterForm() then
		...
		if not isRejuvenation(TargetID) then
			...
			if Zorlen_HasTalent(LOCALIZATION_ZORLEN.ImprovedRejuvenation) then
				HealImproved = HealImproved + (Zorlen_GetTalentRank(LOCALIZATION_ZORLEN.ImprovedRejuvenation) * .05)
			end
...
function castGroupSwiftmend(pet)
	local SpellName = LOCALIZATION_ZORLEN.Swiftmend
	...
	elseif not isCasterForm() then
		...
		if not SpellButton or (( isUsable == 1 ) and ( duration == 0 ) and not ( isCurrent == 1 ) and ( not notEnoughMana )) then
			if SpellButton or Zorlen_checkCooldown(SpellID) then
				local counter = 1
				local notunitarray = {}
				local SpellName1 = LOCALIZATION_ZORLEN.Rejuvenation
				local SpellName2 = LOCALIZATION_ZORLEN.Regrowth
				while counter do
...
function forceSwiftmend()
	...
	if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN.Swiftmend) and not (not UnitExists("target") and (isRejuvenationActive() or isRegrowthActive())) and not (UnitExists("target") and (isRejuvenation() or isRegrowth())) then
...
function forceSelfSwiftmend()
	...
	if Zorlen_IsSpellKnown(LOCALIZATION_ZORLEN.Swiftmend) and not (isRejuvenationActive() or isRegrowthActive()) then
...
function castInnervate()
	local SpellName = LOCALIZATION_ZORLEN.Innervate
	local SpellButton = Zorlen_Button[SpellName]
...
function castBarkskin(SpellRank)
	local SpellName = LOCALIZATION_ZORLEN.Barkskin
	local EnemyTargetNotNeeded = 1
	local BuffName = SpellName
	local SpellCheckNotNeeded = 1
	if not Zorlen_Button_MaxRank[SpellName] and not Zorlen_IsSpellKnown(SpellName) then
...
function castNaturesGrasp(SpellRank)
	local SpellName = LOCALIZATION_ZORLEN.NaturesGrasp
	local EnemyTargetNotNeeded = 1
	local BuffName = SpellName
	local SpellCheckNotNeeded = 1
	if not Zorlen_Button_MaxRank[SpellName] and not Zorlen_IsSpellKnown(SpellName) then
...
function castProwl(SpellRank)
	local SpellName = LOCALIZATION_ZORLEN.Prowl
	local EnemyTargetNotNeeded = 1
	local BuffName = SpellName
	local SpellCheckNotNeeded = 1
	if not Zorlen_Button_MaxRank[SpellName] and not Zorlen_IsSpellKnown(SpellName) then
...
function castNaturesSwiftness(SpellRank)
	local SpellName = LOCALIZATION_ZORLEN.NaturesSwiftness
	local EnemyTargetNotNeeded = 1
	local BuffName = SpellName
	local SpellCheckNotNeeded = 1
	if not Zorlen_Button_MaxRank[SpellName] and not Zorlen_IsSpellKnown(SpellName) then
...
function castEntanglingRoots(SpellRank)
	local SpellName = LOCALIZATION_ZORLEN.EntanglingRoots
...
function castInsectSwarm(SpellRank)
	local SpellName = LOCALIZATION_ZORLEN.InsectSwarm
	local DebuffName = SpellName
	local SpellCheckNotNeeded = 1
	local DebuffTimer = 1
	if not Zorlen_Button_MaxRank[SpellName] and not Zorlen_IsSpellKnown(SpellName) then
...
function castFaerieFire()
	local SpellName = LOCALIZATION_ZORLEN.FaerieFire
	local FeralSpellName = LOCALIZATION_ZORLEN.FaerieFireFeral
...
function castMoonfire(SpellRank, NoDebuffCheck)
	local SpellName = LOCALIZATION_ZORLEN.Moonfire
	local DebuffName = SpellName
	local SpellCheckNotNeeded = 1
	if not Zorlen_Button_MaxRank[SpellName] and not Zorlen_IsSpellKnown(SpellName) then
		return false
...
function castStarfire(SpellRank)
	local SpellName = LOCALIZATION_ZORLEN.Starfire
	local SpellCheckNotNeeded = 1
	if not Zorlen_Button_MaxRank[SpellName] and not Zorlen_IsSpellKnown(SpellName) then
		return false
	end
...
function castWrath(SpellRank)
	local SpellName = LOCALIZATION_ZORLEN.Wrath
	local SpellCheckNotNeeded = 1
	if not Zorlen_Button_MaxRank[SpellName] and not Zorlen_IsSpellKnown(SpellName) then
		return false
	end
