	...
	if Tag == "InternalZorlenSpellTimers" then
		...
		then
			if Name ~= LOCALIZATION_ZORLEN.SerpentSting then
				Zorlen_ClearTimer(LOCALIZATION_ZORLEN.SerpentSting, Pre, Tag)
			end
			if Name ~= LOCALIZATION_ZORLEN.ViperSting then
				Zorlen_ClearTimer(LOCALIZATION_ZORLEN.ViperSting, Pre, Tag)
			end
			if Name ~= LOCALIZATION_ZORLEN.ScorpidSting then
				Zorlen_ClearTimer(LOCALIZATION_ZORLEN.ScorpidSting, Pre, Tag)
			end
			if Name ~= LOCALIZATION_ZORLEN.WyvernSting then
				Zorlen_ClearTimer(LOCALIZATION_ZORLEN.WyvernSting, Pre, Tag)
			end
		end
	end
end
...
function Zorlen_Hunter_OnUpdate()
...
function Zorlen_Hunter_OnEvent_PLAYER_TARGET_CHANGED()
	...
	Zorlen_ClearTimer(LOCALIZATION_ZORLEN.ViperSting, "immune", "InternalZorlenMiscTimer")
	Zorlen_ClearTimer(LOCALIZATION_ZORLEN.ConcussiveShot, "immune", "InternalZorlenMiscTimer")
	Zorlen_ClearTimer(LOCALIZATION_ZORLEN.WingClip, "immune", "InternalZorlenMiscTimer")
...
function Zorlen_Hunter_OnEvent_UI_ERROR_MESSAGE(arg1, arg2, arg3)
	if string.find(arg1, LOCALIZATION_ZORLEN.pet_is_not_dead) then
		Zorlen_debug(LOCALIZATION_ZORLEN.pet_is_not_dead)
		Zorlen_PetIsDead = nil
		ZorlenConfig[ZORLEN_ZPN][ZORLEN_PETISDEAD] = nil
	elseif string.find(arg1, LOCALIZATION_ZORLEN.pet_is_dead) then
		Zorlen_debug(LOCALIZATION_ZORLEN.pet_is_dead)
		Zorlen_PetIsDead = 1
		ZorlenConfig[ZORLEN_ZPN][ZORLEN_PETISDEAD] = true
	end
end
...
function castShotRotation(mode)
	...
	elseif mode ~= "nomulti" and mode ~= "nomultiorfeign" and castMulti() then
		return true
	elseif not UnitIsUnit("player", "targettarget") and (boss or not Zorlen_isDieingEnemy()) and not isQuickshotsActive() and not (isRapidActive() and isHawkActive() and Zorlen_GetTalentRank(LOCALIZATION_ZORLEN.ImprovedAspectOfTheHawk) > 0) and Zorlen_IsTimer("AimedShotRotationWindow", nil, "InternalZorlenMiscTimer") and castAimed() then
		return true
	end
	return false
end
...
function isAutoShotActive()
	local SpellName = LOCALIZATION_ZORLEN.AutoShot
...
function castClip(SpellRank)
	local SpellName = LOCALIZATION_ZORLEN.WingClip
	local DebuffName = SpellName
	local DebuffImmune = Zorlen_IsTimer(SpellName, "immune", "InternalZorlenMiscTimer")
	local r = SpellRank or Zorlen_GetSpellRank(SpellName)
	local mana = UnitMana("player")
...
function castMend(mode)
	...
	local SpellName = LOCALIZATION_ZORLEN.MendPet
	local mana = UnitMana("player")
...
function castCallAndDismissPet()
	if UnitHealth("pet") > 0 then
		...
		CastSpellByName(LOCALIZATION_ZORLEN.DismissPet)
...
-- Added by Corpboy
function castHunterTracking()
	if isBeast() then
		return castTrackBeasts()
	elseif isHumanoid() then
		return castTrackHumanoids()
	elseif isUndead() then
		return castTrackUndead()
	elseif isElemental() then
		return castTrackElementals()
	elseif isDemon() then
		return castTrackDemons()
	elseif isGiant() then
		return castTrackGiants()
	elseif isDragonkin() then
		return castTrackDragonkin()
	end
	return castTrackHumanoids()
end
